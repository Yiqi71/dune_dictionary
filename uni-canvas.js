...
function drawGridAtOffset(offsetX, offsetY, gridSize, lonCount, latCount) {
    for (let lonIdx = 0; lonIdx <= lonCount; lonIdx++) {
        for (let latIdx = 0; latIdx <= latCount; latIdx++) {
            const lon = -180 + lonIdx * lonStep;
            const lat = 90 - latIdx * latStep;
...
function drawSpecialLatLines(offsetX, offsetY, gridSize, latStep, latCount, totalWidth, totalHeight) {
    ...
    const latitudes = [
        { lat: 0, color: "#555", dash: [], lineWidth: 2 },        // 赤道，实线，加粗
        { lat: 23.5, color: "#555", dash: [5, 5], lineWidth: 1 },   // 北回归线，虚线
        { lat: -23.5, color: "#555", dash: [5, 5], lineWidth: 1 },  // 南回归线，虚线
        { lat: 66.5, color: "#555", dash: [5, 5], lineWidth: 1 },  // 北极圈，虚线
        { lat: -66.5, color: "#555", dash: [5, 5], lineWidth: 1 }  // 南极圈，虚线
    ];
